...
GLOSSARY = [
    ...
    # --- Carte et objectifs ---
    ("Carte", "Faille de l'invocateur", "Summoner's Rift",
     "La carte de jeu : trois voies séparées par la jungle, une base à chaque extrémité."),
    ("Carte", "Lane", "Voie",
     "Un des trois chemins qui relient les deux bases : top, mid et bot."),
    ("Carte", "Bush", "Buisson",
     "Zone d'herbes hautes où un champion devient invisible tant qu'un adversaire n'y entre pas. Sert aux embuscades."),
    ("Carte", "Minion", "Sbire",
     "Petite unité envoyée automatiquement par la base. Les tuer rapporte de l'or et de l'expérience."),
    ("Carte", "Wave", "Vague de sbires",
     "Groupe de sbires qui avance sur une voie. « Pousser la wave » = la faire avancer vers l'ennemi."),
    ("Carte", "Farm / CS", "Sbires tués (Creep Score)",
     "Nombre de sbires et monstres tués. Principale source d'or : un bon farm compte autant que les éliminations."),
    ("Carte", "Last hit", "Coup fatal",
     "Porter le dernier coup à un sbire, seule façon d'obtenir son or."),
    ("Carte", "Camp", "Camp de monstres",
     "Groupe de monstres neutres dans la jungle, que le jungler tue pour monter en niveau."),
    ("Carte", "Dragon", "Drake",
     "Monstre neutre majeur de la partie basse. Le tuer donne un bonus permanent à toute l'équipe."),
    ("Carte", "Baron Nashor", "Baron",
     "Monstre neutre le plus puissant. Le tuer renforce l'équipe et ses sbires : souvent décisif en fin de partie."),
    ("Carte", "Héraut de la faille", "Rift Herald",
     "Monstre neutre du début de partie, qui aide à détruire les tourelles."),
    ("Carte", "Tourelle", "Tower / Turret",
     "Défense qui protège chaque voie. Il faut les détruire pour avancer vers la base ennemie."),
    ("Carte", "Inhibiteur", "Inhibitor",
     "Bâtiment protégé par les tourelles. Détruit, il fait apparaître des sbires plus puissants."),
    ("Carte", "Nexus", "Cœur de la base",
     "Bâtiment central de chaque base. Le détruire met fin à la partie."),
    ("Carte", "Fontaine", "Base",
     "Zone de départ où l'on récupère sa vie, son mana et où l'on achète ses objets."),
    # --- Actions de jeu ---
    ("Actions", "Gank", "Embuscade",
     "Arriver par surprise sur une voie, souvent depuis la jungle ou un bush, pour tuer un adversaire à plusieurs."),
    ("Actions", "Roam", "Se déplacer",
     "Quitter sa voie pour aller aider ailleurs sur la carte."),
    ("Actions", "Invade", "Invasion",
     "Entrer dans la jungle ennemie pour lui voler ses camps ou le prendre en embuscade."),
    ("Actions", "Recall / Back", "Rappel",
     "Retourner à la base pour se soigner et acheter des objets."),
    ("Actions", "Ward", "Balise de vision",
     "Objet posé sur la carte pour voir une zone. La vision évite les ganks."),
    ("Actions", "Teamfight", "Combat d'équipe",
     "Affrontement à 5 contre 5, souvent décisif autour d'un objectif."),
    ("Actions", "Trade", "Échange de dégâts",
     "Court échange de coups sur la voie, où l'on cherche à infliger plus de dégâts qu'on en reçoit."),
    ("Actions", "Poke", "Harcèlement",
     "Infliger des dégâts à distance, petit à petit, sans s'engager au corps à corps."),
    ("Actions", "All-in", "Engagement total",
     "Attaquer en utilisant toutes ses compétences, sans possibilité de reculer."),
    ("Actions", "Engage / Disengage", "Engager / Désengager",
     "Lancer un combat d'équipe, ou au contraire le couper pour permettre à son équipe de fuir."),
    ("Actions", "Peel", "Protection",
     "Repousser les ennemis qui attaquent son porteur pour le garder en vie."),
    ("Actions", "Kite", "Harcèlement en reculant",
     "Attaquer tout en reculant, pour infliger des dégâts sans en prendre."),
    ("Actions", "Split push", "Poussée isolée",
     "Pousser seul une voie pendant que le reste de l'équipe occupe les adversaires ailleurs."),
    ("Actions", "Dive", "Plongeon",
     "Attaquer un adversaire sous sa tourelle, en acceptant d'encaisser les tirs."),
    ("Actions", "Freeze", "Gel de la vague",
     "Maintenir la vague de sbires près de sa tourelle pour empêcher l'adversaire de farmer sereinement."),
    ("Actions", "CC (Crowd Control)", "Contrôle de foule",
     "Effets qui empêchent d'agir : étourdissement, ralentissement, immobilisation, etc."),
    ("Actions", "Burst", "Dégâts explosifs",
     "Infliger énormément de dégâts en très peu de temps."),
    ("Actions", "Flash", "Saut éclair",
     "Sort d'invocateur le plus utilisé : téléporte le champion sur une courte distance."),
    # --- Argot et progression ---
    ("Argot", "Early / Mid / Late game", "Début / milieu / fin de partie",
     "Les trois phases d'une partie. Certains champions sont forts tôt, d'autres deviennent forts avec le temps."),
    ("Argot", "Scaling", "Montée en puissance",
     "Capacité d'un champion à devenir plus fort au fil de la partie, avec les niveaux et les objets."),
    ("Argot", "Power spike", "Pic de puissance",
     "Moment où un champion devient nettement plus fort : un niveau clé ou un objet terminé."),
    ("Argot", "Build", "Équipement",
     "Ensemble des objets achetés pendant la partie, choisis selon la situation."),
    ("Argot", "AD / AP", "Dégâts physiques / magiques",
     "Les deux grands types de dégâts : Attack Damage et Ability Power."),
    ("Argot", "Snowball", "Effet boule de neige",
     "Prendre de l'avance et s'en servir pour creuser l'écart jusqu'à la victoire."),
    ("Argot", "Feed", "Nourrir l'adversaire",
     "Mourir souvent, ce qui renforce l'adversaire. Un joueur « fed » est un joueur très en avance."),
    ("Argot", "Tilt", "Perte de sang-froid",
     "État de frustration qui fait enchaîner les mauvaises décisions."),
    ("Argot", "Elo / Rank", "Classement",
     "Niveau d'un joueur en parties classées, du Fer au Challenger."),
    ("Argot", "SoloQ", "File d'attente solo",
     "Mode classé où l'on joue seul, avec des coéquipiers inconnus."),
    ("Argot", "Smurf", "Second compte",
     "Compte secondaire d'un joueur expérimenté, qui affronte des joueurs moins forts."),
    ("Argot", "OTP (One Trick Pony)", "Spécialiste d'un champion",
     "Joueur qui ne joue quasiment qu'un seul champion et le maîtrise très bien."),
    ("Argot", "GG", "Good game",
     "« Belle partie » : message de fin de partie, par politesse ou pour reconnaître la défaite."),
]
